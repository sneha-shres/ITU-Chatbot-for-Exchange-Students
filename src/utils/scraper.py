...
class ITUWebScraper:
    ...
    def extract_page_content(self, url: str, soup: BeautifulSoup) -> Dict:
        ...
        # Extract list items (bullet points) from ul and ol lists
        list_items = soup.find_all('li')
        for li in list_items:
            # Skip list items in footer areas
            if self.is_footer_element(li):
                continue
            
            # Get the text content of the list item
            li_text = self.clean_text(li.get_text())
            # Also check for nested paragraphs within list items
            if not li_text or len(li_text.strip()) < 10:
                # Try to get text from nested p tags
                nested_p = li.find('p')
                if nested_p:
                    li_text = self.clean_text(nested_p.get_text())
            
            if li_text and len(li_text.strip()) > 10:  # Filter out very short list items
                content['paragraphs'].append(li_text)
...
def main():
    ...
    # Scrape all pages
    scraped_data = scraper.scrape_all_pages(max_pages=30) 
